...
@implementation GPUImageFilterPipeline (saveProcessedImageToLibrary)
- (void)saveImageFromCurrentlyProcessedOutputWithMeta:(NSDictionary *)metaData andOrientation:(UIImageOrientation)imageOrientation onComplete:(void(^)(NSURL *assetURL, NSError *error, UIImage *preview))block {
    CGImageRef cgImageFromBytes = [self newCGImageFromCurrentFilteredFrameWithOrientation:imageOrientation];
    UIImage *finalImage = [UIImage imageWithCGImage:cgImageFromBytes scale:1.0 orientation:imageOrientation];
    CGImageRelease(cgImageFromBytes);
    UIImage *preview = [finalImage resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:CGSizeMake(100.0, 100.0) interpolationQuality:kCGInterpolationHigh];
