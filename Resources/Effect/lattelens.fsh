...
{
	vec2 noise = rand(textureCoordinate);
	float tolerance = 0.2;
	float vignette_size = 0.5;
	vec2 powers = pow(abs(vec2(textureCoordinate.s - 0.5,textureCoordinate.t - 0.5)),vec2(2.0));
	...
	col += texture2D(inputImageTexture, vec2(X1, Y1))*0.1;
	col += texture2D(inputImageTexture, vec2(X2, Y2))*0.1;
	col += texture2D(inputImageTexture, vec2(X1, Y2))*0.1;
	col += texture2D(inputImageTexture, vec2(X2, Y1))*0.1;
	
	col += texture2D(inputImageTexture, vec2(invX1, invY1))*0.15;
	col += texture2D(inputImageTexture, vec2(invX2, invY2))*0.15;
	col += texture2D(inputImageTexture, vec2(invX1, invY2))*0.15;
	col += texture2D(inputImageTexture, vec2(invX2, invY1))*0.15;
...
void main(void)
{
	...
	vec2 cancoord = textureCoordinate;
	...
	inputDistort.r = texture2D(inputImageTexture,rCoords).r;
	inputDistort.g = texture2D(inputImageTexture,gCoords).g;
	inputDistort.b = texture2D(inputImageTexture,bCoords).b;
